...
void printRegistersShort(mos6502 *_cpu) {
    fprintf(stdout, " A=0x%02X X=0x%02X Y=0x%02x SP=0x%02X PC=0x%04X FLAGS=0x%02X\n"
...
int stepCpu(mos6502 *_cpu) {
    ...
    printf("%04i) PC: %04X - EXPECTED: %02x, ACTUAL: %02x - %-5s                ", chonkCount, _cpu->PC-1, chonker[chonkCount], opcode, chonker[chonkCount] == opcode?"true":"false");
    printRegistersShort(_cpu);
    chonkCount++;
